...
// TODO: Use what you've learned about how to access keys and values to loop through the object 
// and print only values which belong to food items that are sides. No need to write this one as a function,
//  just do the loop. Hint 1: you need to check the key names, not the values! Hint 2: Even though they don't look like it in the object, keys are stored as strings.
...
function printItemsAtIndex(index) {
	...
	for (let type in pantry) {
		let item = pantry[type][index];
		if (item !== undefined) {
			console.log(`\t${item}`);
		}
	}
...
function printItemsByType(type) {
	if (pantry[type]) {
		...
		for (let item of pantry[type]) {
			console.log(`- ${item}`);
		}
	} else {
		console.log(`No such package type: "${type}"`);
	}
}
...
printItemsByType("boxed");
printItemsByType("frozen");
/*
	Great job!
*/
